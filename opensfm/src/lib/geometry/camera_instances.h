...
/// True for any equidistant-family fisheye projection.
inline bool IsFisheye(ProjectionType t) {
  return t == ProjectionType::FISHEYE || t == ProjectionType::FISHEYE_OPENCV ||
         t == ProjectionType::FISHEYE62 || t == ProjectionType::FISHEYE624;
}
...
struct RigPoseNormalizedDerivatives {
  template <class TYPE, class T>
  static void Apply(const T* point, const T* parameters, T* projected,
                    T* jacobian) {
    ComposeForwardDerivatives<T, true, Normalize, PoseFunctor, PoseFunctor>(
        point, parameters, projected, jacobian);
  }
};
